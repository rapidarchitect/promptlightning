#!/usr/bin/env python3
"""
Test that promptvault init command works correctly
"""
import tempfile
import os
...
# Add parent directory to path to import promptvault
...
def test_init_creates_proper_structure():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            config_path = Path("promptvault.yaml")
            assert config_path.exists(), "promptvault.yaml was not created"
            ...
            assert config_data["logging"]["db_path"] == "./promptvault.db", "Wrong db path"
...
def test_init_output_message():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            result = runner.invoke(app, ["init"])
            assert result.exit_code == 0, f"Init command failed: {result.stdout}"
            assert "Initialized PromptVault project." in result.stdout, f"Wrong output message: {result.stdout}"
...
def test_init_works_in_existing_directory():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert Path("promptvault.yaml").exists(), "Config file was not created"
...
def main():
    """Run all init tests"""
    print("🧪 Testing promptvault init command...\n")
